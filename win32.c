...
{
    FF(GetVolumeInformationA, -1, uFppuuuupu, 32)
    FF(GetDriveTypeA, -1, uFp, 4)
    FF(GetLogicalDriveStringsA, -1, uFup, 8)
    FF(IsBadWritePtr, 357, uFpu, 8)
    FF(IsBadReadPtr, 354, uFpu, 8)
    FF(IsBadStringPtrW, -1, uFpu, 8)
    FF(IsBadStringPtrA, -1, uFpu, 8)
    FF(DisableThreadLibraryCalls, -1, uFi, 4)
    FF(CreateThread, -1, iFpuppuu, 24)
    FF(ResumeThread, -1, uFi, 4)
    FF(CreateEventA, -1, iFpuup, 16)
    FF(CreateEventW, -1, iFpuup, 16)
    FF(SetEvent, -1, uFi, 4)
    FF(ResetEvent, -1, uFi, 4)
    FF(WaitForSingleObject, -1, uFiu, 8)
#ifdef CONFIG_QTX_CODECS
    FF(WaitForMultipleObjects, -1, uFuiuu, 16)
    FF(ExitThread, -1, vFu, 4)
#endif
    FF(GetSystemInfo, -1, vFp, 4)
    FF(GetVersion, 332, iFv, 0)
    FF(HeapCreate, 461, iFuuu, 12)
    FF(HeapAlloc, -1, pFiuu, 12)
    FF(HeapDestroy, -1, uFi, 4)
    FF(HeapFree, -1, uFiup, 12)
    FF(HeapSize, -1, uFiup, 12)
    FF(HeapReAlloc, -1, pFiupu, 16)
    FF(HeapValidate, -1, uFiup, 12)
    FF(GetProcessHeap, -1, iFv, 0)
    FF(VirtualAlloc, -1, pFpuuu, 16)
    FF(VirtualFree, -1, uFpuu, 12)
    FF(InitializeCriticalSection, -1, vFp, 4)
    FF(InitializeCriticalSectionAndSpinCount, -1, vFpu, 8)
    FF(EnterCriticalSection, -1, vFp, 4)
    FF(LeaveCriticalSection, -1, vFp, 4)
    FF(DeleteCriticalSection, -1, vFp, 4)
    FF(TlsAlloc, -1, uFv, 0)
    FF(TlsFree, -1, uFu, 4)
    FF(TlsGetValue, -1, pFu, 4)
    FF(TlsSetValue, -1, uFup, 8)
    FF(GetCurrentThreadId, -1, uFv, 0)
    FF(GetCurrentProcess, -1, iFv, 0)
    FF(LocalAlloc, -1, iFuu, 8)
    FF(LocalReAlloc, -1, iFiuu, 12)
    FF(LocalLock, -1, pFi, 4)
    FF(GlobalAlloc, -1, iFuu, 8)
    FF(GlobalReAlloc, -1, iFiuu, 12)
    FF(GlobalLock, -1, pFi, 4)
    FF(GlobalSize, -1, uFi, 4)
    FF(MultiByteToWideChar, 427, iFuupipi, 24)
    FF(WideCharToMultiByte, -1, iFuupipipu, 32)
    FF(GetVersionExA, -1, uFp, 4)
    FF(GetVersionExW, -1, uFp, 4)
    FF(CreateSemaphoreA, -1, iFpiip, 16)
    FF(CreateSemaphoreW, -1, iFpiip, 16)
    FF(QueryPerformanceCounter, -1, uFp, 4)
    FF(QueryPerformanceFrequency, -1, uFp, 4)
    FF(LocalHandle, -1, iFp, 4)
    FF(LocalUnlock, -1, uFi, 4)
    FF(LocalFree, -1, iFi, 4)
    FF(GlobalHandle, -1, iFp, 4)
    FF(GlobalUnlock, -1, uFi, 4)
    FF(GlobalFree, -1, iFi, 4)
    FF(LoadResource, -1, iFii, 8)
    FF(ReleaseSemaphore, -1, uFiip, 12)
    FF(CreateMutexA, -1, iFpup, 12)
    FF(CreateMutexW, -1, iFpup, 12)
    FF(ReleaseMutex, -1, uFi, 4)
    FF(SignalObjectAndWait, -1, vFv, 0)
    FF(FindResourceA, -1, iFipp, 12)
    FF(LockResource, -1, pFi, 4)
    FF(FreeResource, -1, vFv, 0)
    FF(SizeofResource, -1, uFii, 8)
    FF(CloseHandle, -1, uFi, 4)
    FF(GetCommandLineA, -1, pFv, 0)
    FF(GetEnvironmentStringsW, -1, pFv, 0)
    FF(FreeEnvironmentStringsW, -1, uFp, 4)
    FF(FreeEnvironmentStringsA, -1, iFv, 0)
    FF(GetEnvironmentStrings, -1, pFp, 4)
    FF(GetStartupInfoA, -1, vFp, 4)
    FF(GetStdHandle, -1, iFu, 4)
    FF(GetFileType, -1, uFi, 4)
#ifdef CONFIG_QTX_CODECS
    FF(GetFileAttributesA, -1, uFp, 4)
#endif
    FF(SetHandleCount, -1, uFu, 4)
    FF(GetACP, -1, uFv, 0)
    FF(GetModuleFileNameA, -1, uFipu, 12)
    FF(SetUnhandledExceptionFilter, -1, pFp, 4)
    FF(LoadLibraryA, -1, iFp, 4)
    FF(GetProcAddress, -1, pFip, 8)
    FF(FreeLibrary, -1, uFi, 4)
    FF(CreateFileMappingA, -1, iFipuuup, 24)
    FF(OpenFileMappingA, -1, iFuup, 12)
    FF(MapViewOfFile, -1, pFiuuuu, 20)
    FF(UnmapViewOfFile, -1, uFp, 4)
    FF(Sleep, -1, vFu, 4)
    FF(GetModuleHandleA, -1, iFp, 4)
    FF(GetModuleHandleW, -1, iFp, 4)
    FF(GetProfileIntA, -1, uFppi, 12)
    FF(GetPrivateProfileIntA, -1, uFppip, 16)
    FF(GetPrivateProfileStringA, -1, iFppppup, 24)
    FF(WritePrivateProfileStringA, -1, uFpppp, 16)
    FF(GetLastError, -1, uFv, 0)
    FF(SetLastError, -1, vFu, 4)
    FF(InterlockedIncrement, -1, iFp, 4)
    FF(InterlockedDecrement, -1, iFp, 4)
    FF(GetTimeZoneInformation, -1, pFp, 4)
    FF(OutputDebugStringA, -1, vFp, 4)
    FF(GetLocalTime, -1, vFp, 4)
    FF(GetSystemTime, -1, vFp, 4)
    FF(GetSystemTimeAsFileTime, -1, vFp, 4)
    FF(GetEnvironmentVariableA, -1, uFppu, 12)
    FF(SetEnvironmentVariableA, -1, uFpp, 8)
    FF(RtlZeroMemory, -1, pFp, 4)
    FF(RtlMoveMemory, -1, pFp, 4)
    FF(RtlFillMemory, -1, pFp, 4)
    FF(GetTempPathA, -1, uFup, 8)
    FF(FindFirstFileA, -1, iFpp, 8)
    FF(FindNextFileA, -1, uFip, 8)
    FF(FindClose, -1, uFi, 4)
    FF(FileTimeToLocalFileTime, -1, uFpp, 8)
    FF(DeleteFileA, -1, uFp, 4)
    FF(ReadFile, -1, uFipuup, 20)
    FF(WriteFile, -1, uFipuup, 20)
    FF(SetFilePointer, -1, uFiipu, 16)
    FF(GetTempFileNameA, -1, uFppup, 16)
    FF(CreateFileA, -1, iFpuupuui, 28)
    FF(GetSystemDirectoryA, -1, uFpu, 8)
    FF(GetWindowsDirectoryA, -1, uFpu, 8)
    FF(GetCurrentDirectoryA, -1, uFup, 8)
    FF(SetCurrentDirectoryA, -1, uFp, 4)
    FF(CreateDirectoryA, -1, uFpp, 8)
    FF(GetShortPathNameA, -1, uFppu, 12)
    FF(GetFullPathNameA, -1, uFpupp, 16)
    FF(SetErrorMode, -1, uFu, 4)
    FF(IsProcessorFeaturePresent, -1, uFu, 4)
    FF(IsDebuggerPresent, -1, uFv, 0)
    FF(GetProcessAffinityMask, -1, vFv, 0)
    FF(InterlockedExchange, -1, iFpi, 8)
    FF(InterlockedCompareExchange, -1, pFppp, 12)
    FF(MulDiv, -1, pFp, 4)
    FF(lstrcmpiA, -1, iFpp, 8)
    FF(lstrlenA, -1, iFp, 4)
    FF(lstrlenW, -1, iFp, 4)
    FF(lstrcpyA, -1, pFpp, 8)
    FF(lstrcatA, -1, pFpp, 8)
    FF(lstrcpynA, -1, pFppi, 12)
    FF(GetProcessVersion, -1, uFu, 4)
    FF(GetCurrentThread, -1, iFv, 0)
    FF(GetOEMCP, -1, uFv, 0)
    FF(GetCPInfo, -1, pFp, 4)
    FF(DuplicateHandle, -1, uFiiiiuuu, 28)
    FF(GetTickCount, -1, pFv, 0)
    FF(SetThreadAffinityMask, -1, uFiu, 8)
    FF(GetCurrentProcessId, -1, pFv, 0)
    FF(GlobalMemoryStatus, -1, vFp, 4)
    FF(GetThreadPriority, -1, iFi, 4)
    FF(SetThreadPriority, -1, uFii, 8)
    FF(TerminateProcess, -1, uFiu, 8)
    FF(ExitProcess, -1, vFu, 4)
    WRFF(LoadLibraryExA, -1, uFpuu, 12)
    FF(SetThreadIdealProcessor, -1, pFp, 4)
    FF(SetProcessAffinityMask, -1, pFp, 4)
    FF(EncodePointer, -1, pFp, 4)
    FF(DecodePointer, -1, pFp, 4)
    FF(GetThreadLocale, -1, uFv, 0)
    FF(GetLocaleInfoA, -1, iFuupi, 16)
    UNDEFF(FlsAlloc, -1)
...
static const struct exports exp_msvcrt[]={
    FF(malloc, -1, pFi, 0)
    FF(_initterm, -1, iFpp, 0)
    FF(__dllonexit, -1, pFv, 0)
    FF(_snprintf, -1, iFv, 0)
    FF(free, -1, vFp, 0)
    WR2FF("??3@YAXPAX@Z", -1, expdelete, vFp, 0)
    WR2FF("??2@YAPAXI@Z", -1, expnew, pFu, 0)
    WR2FF("_adjust_fdiv", -1, &_adjust_fdiv, 0, 0)
    WR2FF("_winver", -1, &_winver, 0, 0)
    WR2FF("_write", -1, (void*)&write, iFipu, 0)
    WR2FF("_open", -1, (void*)&open, iFpu, 0)
    WR2FF("_close", -1, (void*)&close, iFi, 0)
    WR2FF("_getcwd", -1, (void*)&getcwd, pFv, 0)
    WR2FF("_wgetcwd", -1, (void*)&getcwd, pFv, 0)
    WR2FF("_stat", -1, (void*)&stat, iFpp, 0)
    WR2FF("_unlink", -1, (void*)&unlink, iFp, 0)
    FF(_errno, -1, iFv, 0)
    FF(strrchr, -1, pFpu, 0)
    FF(strchr, -1, pFpu, 0)
    FF(strlen, -1, iFp, 0)
    FF(strcpy, -1, pFpp, 0)
    FF(strncpy, -1, pFppu, 0)
    //FF(strncat, -1, -1, 0)
    FF(wcscpy, -1, pFpp, 0)
    FF(strcmp, -1, iFpp, 0)
    FF(strncmp, -1, iFp, 0)
    ...
    //FF(strpbrk, -1, -1, 0)
    //FF(strerror, -1, -1, 0)
    WRFF(strncat, -1, uFpp, 0)
    WRFF(strtol, -1, uFp, 0)
    WRFF(strspn, -1, pFpp, 0)
    WRFF(strpbrk, -1, pFpp, 0)
    WRFF(strerror, -1, pFu, 0)
    WRFF(exit, -1, vFi, 0)
    WRFF(ctime, -1, uFu, 0)
    WRFF(abort, -1, vFp, 0)
    
    FF(_stricmp, -1, iFpp, 0)
    FF(_strnicmp, -1, iFppu, 0)
    FF(_strdup, -1, pFp, 0)
    FF(_setjmp3, -1, iFp, 0)
    FF(isalnum, -1, iFi, 0)
    FF(isspace, -1, iFi, 0)
    FF(isalpha, -1, iFi, 0)
    FF(isdigit, -1, iFi, 0)
    WRFF(isupper, -1, iFc, 0)
    FF(memmove, -1, pFppu, 0)
    FF(memcmp, -1, iFppu, 0)
    FF(memset, -1, pFpiu, 0)
    FF(memcpy, -1, pFppu, 0)
    FF(time, -1, uFp, 0)
    ...
    FF(ldexp, -1, dFdi, 0)
    FF(frexp, -1, dFdp, 0)
    FF(sprintf, -1, iFppp, 0)
    FF(sscanf, -1, iFpppppp, 0)
    FF(fopen, -1, pFpp, 0)
    FF(fclose, -1, iFp, 0)
    FF(fwrite, -1, iFp, 0)
    WRFF(fgets, -1, pFp, 0)
...
/* needed by frapsvid.dll */
    WRFF(strstr, -1, pFpp, 0)
    WRFF(qsort, -1, iFpppp, 0)
    FF(_EH_prolog, -1, vFv, 0)
    FF(calloc, -1, pFuu, 0)
    WRFF(ceil, -1, iFd, 0)
/* needed by imagepower mjpeg2k */
    WRFF(clock, -1, uFu, 0)
    WRFF(memchr, -1, iFpuu, 0)
    WRFF(vfprintf, -1, iFppp, 0)
    WR2FF("_vsnprintf", -1, (void*)&vsnprintf, iFupp, 0)
    WR2FF("_vsprintf", -1, (void*)&vsprintf, iFpp, 0)
    WRFF(vsprintf, -1, iFpp, 0)
    WRFF(strtok, -1, pFpp, 0)
//    {"realloc",-1,(void*)&realloc},
    FF(realloc, -1, vFv, 0)
    WRFF(puts, -1, vFp, 0)
};
static const struct exports exp_winmm[]={
    FF(GetDriverModuleHandle, -1, iFi, 4)
    FF(timeGetTime, -1, vFv, 0)
    FF(DefDriverProc, -1, vFv, 0)
    FF(OpenDriverA, -1, iFppp, 12)
    FF(OpenDriver, -1, vFv, 0)
    FF(timeGetDevCaps, -1, vFv, 0)
    FF(timeBeginPeriod, -1, vFv, 0)
    FF(joyGetNumDevs, -1, vFv, 0)
#ifdef CONFIG_QTX_CODECS
    FF(timeEndPeriod, -1, vFv, 0)
    FF(waveOutGetNumDevs, -1, vFv, 0)
#endif
};
static const struct exports exp_psapi[]={
    FF(GetModuleBaseNameA, -1, vFv, 0)
};
static const struct exports exp_user32[]={
    FF(LoadIconA, -1, vFv, 0)
    FF(LoadStringA, -1, vFv, 0)
    FF(wsprintfA, -1, iFp, 0)
    FF(GetDC, -1, vFv, 0)
    FF(GetDesktopWindow, -1, vFv, 0)
    FF(ReleaseDC, -1, vFv, 0)
    FF(IsRectEmpty, -1, vFv, 0)
    FF(LoadCursorA, -1, vFv, 0)
    FF(SetCursor, -1, vFv, 0)
    FF(GetCursorPos, -1, vFv, 0)
    FF(SetCursorPos, -1, vFv, 0)
    FF(ShowCursor, -1, vFv, 0)
    FF(RegisterWindowMessageA, -1, vFv, 0)
    FF(GetSystemMetrics, -1, vFv, 0)
    FF(GetSysColor, -1, vFv, 0)
    FF(GetSysColorBrush, -1, vFv, 0)
    FF(GetWindowDC, -1, vFv, 0)
    FF(DrawTextA, -1, vFv, 0)
    FF(MessageBoxA, -1, uFpppp, 16)
    FF(RegisterClassA, -1, vFv, 0)
    FF(UnregisterClassA, -1, vFv, 0)
    FF(GetWindowRect, -1, vFv, 0)
    FF(MonitorFromWindow, -1, vFv, 0)
    FF(MonitorFromRect, -1, vFv, 0)
    FF(MonitorFromPoint, -1, vFv, 0)
    FF(EnumDisplayMonitors, -1, vFv, 0)
    FF(GetMonitorInfoA, -1, vFv, 0)
    FF(EnumDisplayDevicesA, -1, vFv, 0)
//    FF(GetClientRect, -1, vFv, 0)
//    FF(ClientToScreen, -1, vFv, 0)
    FF(IsWindowVisible, -1, vFv, 0)
    FF(GetActiveWindow, -1, vFv, 0)
    FF(GetClassNameA, -1, vFv, 0)
    FF(GetClassInfoA, -1, vFv, 0)
    FF(GetWindowLongA, -1, vFv, 0)
    FF(EnumWindows, -1, vFv, 0)
    FF(GetWindowThreadProcessId, -1, vFv, 0)
    FF(CreateWindowExA, -1, vFv, 0)
    FF(MessageBeep, -1, vFv, 0)
    FF(DialogBoxParamA, -1, vFv, 0)
    FF(RegisterClipboardFormatA, -1, vFv, 0)
    FF(CharNextA, -1, vFv, 0)
    FF(EnumDisplaySettingsA, -1, vFv, 0)
    FF(SystemParametersInfoA, -1, vFv, 0)
    FF(PeekMessageA, -1, vFv, 0)
};
static const struct exports exp_advapi32[]={
    FF(RegCloseKey, -1, vFv, 0)
    FF(RegCreateKeyA, -1, vFv, 0)
    FF(RegCreateKeyExA, -1, vFv, 0)
    FF(RegEnumKeyExA, -1, vFv, 0)
    FF(RegEnumValueA, -1, vFv, 0)
    FF(RegOpenKeyA, -1, vFv, 0)
    FF(RegOpenKeyExA, -1, vFv, 0)
    FF(RegQueryValueExA, -1, vFv, 0)
    FF(RegSetValueExA, -1, vFv, 0)
    FF(RegQueryInfoKeyA, -1, vFv, 0)
};
static const struct exports exp_gdi32[]={
    FF(CreateCompatibleDC, -1, vFv, 0)
    FF(CreateFontA, -1, vFv, 0)
    FF(DeleteDC, -1, vFv, 0)
    FF(DeleteObject, -1, vFv, 0)
    FF(GetDeviceCaps, -1, vFv, 0)
    FF(GetSystemPaletteEntries, -1, vFv, 0)
    FF(CreatePalette, -1, iFv, 0)
    FF(GetObjectA, -1, vFv, 0)
    FF(GetCharABCWidthsA, -1, vFv, 0)
    FF(SetTextAlign, -1, vFv, 0)
    FF(GetTextMetricsA, -1, vFv, 0)
    FF(CreateDIBSection, -1, vFv, 0)
    FF(SelectObject, -1, vFv, 0)
    FF(CreateRectRgn, -1, vFv, 0)
};
static const struct exports exp_version[]={
    FF(GetFileVersionInfoSizeA, -1, pFp, 4)
};
static const struct exports exp_ole32[]={
    FF(CoCreateFreeThreadedMarshaler, -1, vFv, 0)
    FF(CoCreateInstance, -1, vFv, 0)
    FF(CoInitialize, -1, vFv, 0)
    FF(CoInitializeEx, -1, vFv, 0)
    FF(CoUninitialize, -1, vFv, 0)
    FF(CoTaskMemAlloc, -1, vFv, 0)
    FF(CoTaskMemFree, -1, vFv, 0)
    FF(StringFromGUID2, -1, vFv, 0)
    FF(PropVariantClear, -1, vFv, 0)
};
...
static const struct exports exp_comctl32[]={
    FF(StringFromGUID2, -1, vFv, 0)
    FF(InitCommonControls, 17, vFv, 0)
#ifdef CONFIG_QTX_CODECS
    FF(CreateUpDownControl, 16, vFv, 0)
#endif
};
...
static const struct exports exp_msdmo[]={
    FF(memcpy, -1, pFppu, 0) // just test
    FF(MoCopyMediaType, -1, vFv, 0)
    FF(MoCreateMediaType, -1, vFv, 0)
    FF(MoDeleteMediaType, -1, vFv, 0)
    FF(MoDuplicateMediaType, -1, vFv, 0)
    FF(MoFreeMediaType, -1, vFv, 0)
    FF(MoInitMediaType, -1, vFv, 0)
};
static const struct exports exp_oleaut32[]={
    FF(SysAllocStringLen, -1, vFv, 0)
    FF(SysFreeString, 6, vFv, 0)
    FF(VariantInit, 8, vFv, 0)
#ifdef CONFIG_QTX_CODECS
    FF(SysStringByteLen, 149, iFv, 0)
#endif
};
...
static const struct exports exp_pncrt[]={
    FF(malloc, -1, pFi, 0) // just test
    FF(free, -1, vFp, 0) // just test
    FF(fprintf, -1, iFpp, 0) // just test
    WR2FF("_adjust_fdiv", -1, &_adjust_fdiv, 0, 0)
    FF(_ftol, -1, IFv, 0)
    FF(_initterm, -1, iFpp, 0)
    WR2FF("??3@YAXPAX@Z", -1, expdelete, vFp, 0)
...
static const struct exports exp_ddraw[]={
    FF(DirectDrawCreate, -1, iFv, 0)
};
#endif

static const struct exports exp_comdlg32[]={
    FF(GetOpenFileNameA, -1, uFp, 4)
};

static const struct exports exp_shlwapi[]={
    FF(PathFindExtensionA, -1, pFp, 4)
    FF(PathFindFileNameA, -1, pFp, 4)
};
...
static const struct exports exp_msvcr80[]={
    ...
    FF(malloc, -1, pFi, 0)
    FF(free, -1, vFp, 0)
    FF(_initterm_e, -1, iFpp, 0)
    FF(_initterm, -1, iFpp, 0)
    FF(_decode_pointer, -1, iFv, 0)

// For CFDecode2.ax
    WR2FF("_aligned_free", -1, (void*)expfree, pFu, 0)
    WR2FF("_aligned_malloc", -1, (void*)expmalloc, pFu, 0)
    FF(_splitpath, -1, vFc, 0)
    FF(_mbsupr, -1, cFc, 0)
    WR2FF("_mbscmp", -1, (void*)strcmp, iFpp, 0)
    WRFF(clock, -1, iFv, 0)
    FF(_time64, -1, IFv, 0)
...
static const struct exports exp_msvcp60[]={
    WR2FF("??0_Lockit@std@@QAE@XZ", -1, exp_0Lockit_dummy, vFv, 0)
    WR2FF("??1_Lockit@std@@QAE@XZ", -1, exp_1Lockit_dummy, vFv, 0)
};
...
static const struct exports exp_sdl2[]={
    FF(SDL_GetRelativeMouseState, -1, vFpp, 0)
    FF(SDL_NumJoysticks, -1, iFv, 0)
    FF(SDL_GameControllerGetAxis, -1, wFpi, 0)
};

static const struct exports exp_opengl32[]={
    FF(wglGetProcAddress, -1, pFp, 4)
};
...
void* LookupExternal(const char* library, int ordinal)
{
 ...
 //       printf("%x %x\n", &unk_exp1, &unk_exp2);
	...
	for(j=0; j<libraries[i].length; j++)
	{
        ...
        if( !w ) w = vFv;
        return AddCheckBridge(dllbridge,w, libraries[i].exps[j].func, libraries[i].exps[j].asize);
#else
